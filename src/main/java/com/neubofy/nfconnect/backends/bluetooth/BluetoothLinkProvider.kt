 ...
 */
package com.neubofy.nfconnect.backends.bluetooth
...
import com.neubofy.nfconnect.ui.SettingsFragment
import com.neubofy.nfconnect.extensions.getParcelableArrayCompat
...
class BluetoothLinkProvider(private val context: Context) : BaseLinkProvider() {
    ...
    private inner class ServerRunnable : Runnable {
        ...
        override fun run() {
            serverSocket = try {
                bluetoothAdapter!!.listenUsingRfcommWithServiceRecord("NF Connect", SERVICE_UUID)
            } catch (e: IOException) {
                Log.e("NfConnect", "Exception", e)
                return
    ...
    private inner class ClientConnect(private val device: BluetoothDevice?) : Runnable {
        ...
        private fun connectToDevice() {
            ...
            try {
                ...
                Log.e("BTLinkProvider/Client", "Could not connect to NF Connect service on " + device!!.address, e)
                return
            } catch (e: SecurityException) {
